        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="absolute top-full right-0 mt-2 w-48 border border-foreground bg-background z-[101]"
            >
              ...
              {themes.map(theme => (
                <button
                  key={theme.id}
                  onClick={() => { onThemeSelect(theme.id); setIsOpen(false); }}
                  className={`w-full flex items-center gap-3 px-3 py-2 hover:bg-muted transition-colors text-left ${currentTheme === theme.id ? 'bg-muted' : ''}`}
                >
                  <div className="w-5 h-5 border border-foreground/50 flex-shrink-0" style={{ background: theme.preview }} />
                  <span className="text-xs font-mono uppercase tracking-wider">{theme.name}</span>
                </button>
              ))}
            </motion.div>
          </>
